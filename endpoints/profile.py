...
from utility.image import create_avatar, get_image_response, get_image_asset, get_font_optimal, get_font_asset, \
    get_text_newlined, resize_to_ratio, draw_ellipse, for_each_frame, extract_and_resize_to_ratio
...
class ProfileHandler(SingleImageHandler):
    ...
    def on_request(self, avatar: Image):
        ...
        if banner_id is None:
            ...
            return get_image_response([generate_profile(image)])
        else:
            with Image.open(ProfileHandler.DIRECTORY.format(directory) + banner_id) as image:
                def parse(frame):
                    ...
                    return generate_profile(frame)

                return get_image_response(for_each_frame(image, parse) if gif else [parse(image)])
